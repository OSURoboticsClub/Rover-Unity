...
def detect_orange_hammer(image):
    ...
    blurred = image
    #blurred = cv2.bilateralFilter(image, 9, 85, 75)
...
# average_hsv = get_average_hsv("it2.jpg")
# print(f"Average HSV: {average_hsv}")
i = 0
while True:
    random_img_path = get_random_image("Utah/", i)
    i = i + 1
    random_img = cv2.imread(random_img_path)
    #random_img = resize_image(random_img)
    # cv2.imshow("Detected Hammer", random_img)
    # cv2.waitKey(0)
    # cv2.destroyAllWindows()

    # image = color_cluster_segmentation(random_img)
    x,y,size,image = detect_orange_hammer(random_img)
    
    image = resize_image(image)
    cv2.imshow("Detected Hammer", image)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
